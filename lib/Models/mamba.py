...
class MAMBA_Config(PretrainedConfig):
	model_type = "MAMBA"
	def __init__(self,forward_expansion:float=1, kernel_size:int=32, d_state:int = 32, max_length:int=256, **kwargs):
...
class MAMBA(PreTrainedModel):
	...
	def __init__(self, config:MAMBA_Config):
		...
		config = MambaConfig(d_model=config.hidden_size, 
							 n_layers=config.num_hidden_layers,
							 expand_factor=config.forward_expansion,
							 d_state=config.d_state,
							 d_conv=config.kernel_size,
							 use_cuda=True)
