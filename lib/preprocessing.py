...
def dataset_selector(name: str, reduced: bool = False):
    ...
    if name == "imdb":  # https://huggingface.co/datasets/stanfordnlp/imdb
        ...
        dataset = {"text": text_train, "label": label_train}
        ...
        url = "https://raw.githubusercontent.com/ilmgut/limit_dataset/refs/heads/master/data/train.json"
        if not os.path.exists(dataset_path):
            print(f"Downloading limit dataset from {url}...")
        ...
        for item in data.values():
            text.append(item["sentence"])
            label.append(item["motion"])
        ...
        train_data = train_data.map(
            lambda sample: {"label": label_to_id[sample["category"]]}, batched=False
        )
        ...
        train_data = train_data.map(
            lambda x: {"text": x["question"] + " [SEP] " + x["sentence"]}
        )
        test_data = test_data.map(
            lambda x: {"text": x["question"] + " [SEP] " + x["sentence"]}
        )
    ...
    # STARTIN GLUE DATASET
        ...
        # join premise and hypotesis in the same text with a separator
        train_data = train_data.map(
            lambda x: {"text": x["premise"] + " [SEP] " + x["hypothesis"]}
        )
        test_data = test_data.map(
            lambda x: {"text": x["premise"] + " [SEP] " + x["hypothesis"]}
        )
        ...
        # join premise and hypotesis in the same text with a separator
        train_data = train_data.map(
            lambda x: {"text": x["question1"] + " [SEP] " + x["question2"]}
        )
        test_data = test_data.map(
            lambda x: {"text": x["question1"] + " [SEP] " + x["question2"]}
        )
        ...
        name == "stsb"
    ):  # https://huggingface.co/datasets/nyu-mll/multi_nli  # 		WARNING THIS DATASET IS FOR REGRESSION NOT CLASIFICATION
...
def make_tokenizer(
    tokenizer_type: str, dictionary_size: int, dataset_name: str, train_dataset: Dataset
):
    try:
        tokenizer = Tokenizer.from_file(
            f"tokenizers/{tokenizer_type}_{dataset_name}_{dictionary_size}.json"
        )
        ...
        if tokenizer_type == "bpe":
            trainer = BpeTrainer(
                special_tokens=special_tokens, vocab_size=dictionary_size
            )
        elif tokenizer_type == "wordpiece":
        ...
        try:
            ...
            print(
                f"{FOREGROUND_COLORS['BrightRed']}DICTIONARY SIZE TOO BIG FOR DATASET, RESORTING TO LOWER SIZE{RESET}"
            )
...
def pretr_tokenizer(dataset: Dataset, dictionary_size: int, max_length: int):
    try:
        tokenizer = Tokenizer.from_file(
            f"tokenizers/bpe_book_corpus_{dictionary_size}.json"
        )
        ...
        if tokenizer.get_vocab_size() != dictionary_size:
            print(
                f"{FOREGROUND_COLORS['BrightRed']}DICTIONARY SIZE TOO BIG FOR DATASET, RESORTING TO LOWER SIZE {tokenizer.get_vocab_size()}{RESET}"
            )
