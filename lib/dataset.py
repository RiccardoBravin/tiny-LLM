...
def dataset_importer(dataset_name, vocab_size, max_length = 512, batch_size = 32, custom_sentencepiece = None):
	...
	if not os.path.exists("./stpiece/m_" + str(vocab_size) + "_" + dataset_name + ".model") and custom_sentencepiece is None:
		#Train tokenizer
		spm.SentencePieceTrainer.train(input="./stpiece/train_ds_" + dataset_name + ".txt", model_prefix="./stpiece/m_" + str(vocab_size) + "_" + dataset_name, max_sentence_length = 100000000 ,vocab_size=vocab_size)
	elif custom_sentencepiece is not None:
		dataset_name = custom_sentencepiece

	#Load tokenizer
	sp = spm.SentencePieceProcessor(model_file="./stpiece/m_" + str(vocab_size) + "_" + dataset_name + ".model")
