...
class Reader(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(x.size(1)-1):
            w = torch.einsum('bi,j->bij', x[:,i,:], self.weighter) + self.eps # (batch_size, d_model, d_model) 

            aux = torch.einsum('bi,bij->bi', x[:,i+1,:], w).expand_as(x[:,i+1,:])
            x[:,i+1,:] = torch.nn.functional.normalize(aux, p=2, dim=1).expand_as(x[:,i+1,:])
